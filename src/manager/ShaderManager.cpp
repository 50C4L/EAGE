...
//--------------------------------------------------------------------------------------
// ShaderTask definitions
ShaderTask::ShaderTask( char* data, int shaderId )
{
	_data = data;
	_shaderId = shaderId;
}
...
bool ShaderTask::run()
{
	return ShaderManager::getInstance()->loadFromMemory( _data, _shaderId );
}



//======================================================================================
// ShaderManager definitions
...
ShaderManager::ShaderManager()
{
	...
	_currentBindedHandle = 0;

	_shaderTaskMan = new TaskManager();
}
...
void ShaderManager::appendShaderTask( ShaderTask* task )
{
	_shaderTaskMan->appendTask( task );
}


void ShaderManager::executeTasks()
{
	_shaderTaskMan->begin();
}
...
void ShaderManager::bind( int shaderId )
{
	...
	{
		int tempId = _shaderList[shaderId]->getProgramId();
		if( _currentBindedHandle != tempId )
		{
			_currentBindedHandle = tempId;
			glUseProgramObjectARB( _currentBindedHandle );
		}
	}	
	else
		glUseProgramObjectARB( _shaderList[DEFAULT_SHADER]->getProgramId() );
}
