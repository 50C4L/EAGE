...
bool ShaderObject::loadShaderFromMemory( char* shaderContent )
{
	if( shaderContent )
	{
		std::stringstream	stream( shaderContent );
		char				key[32];
			...
			if( strcmp( key, "#ifndef" ) == 0 )
			{
				...
				if( strcmp( key, "VERTEX" ) == 0 )
				{
					...
					if( !shader.empty() )
					{
						...
						{
							eage::debug( "<ERROR> ShaderObject: Failed to compile vertex shader.\n" );
						}
					}
					else
						eage::debug( "<ERROR> ShaderObject: No vertex shader has been found.\n" );
				}
				...
				else if( strcmp( key, "FRAGMENT" ) == 0 )
				{
					...
					if( !shader.empty() )
					{
						...
						{
							eage::debug( "<ERROR> ShaderObject: Failed to compile fragment shader.\n" );
						}
					}
					else
						eage::debug( "<ERROR> ShaderObject: No fragment shader has been found.\n" );
				}
			}
		}
		...
		if( _vertCom && _fragCom )
		{
			...
			{
				eage::debug( "<ERROR> ShaderObject: Failed to link shader program.\n" );
			}
		}
		else
		{
		...
		eage::debug( "ShaderObject: Successfully linked shader program, ID: %d\n", _prog );
	}
	return true;
}
...
bool ShaderObject::loadShader( std::string shaderName )
{
		...
		if( strcmp( key, "#ifndef" ) == 0 )
		{
			...
			if( strcmp( key, "VERTEX" ) == 0 )
			{
				...
				if( !shader.empty() )
				{
					...
					{
						eage::debug( "<ERROR> ShaderObject: [%s], failed to compile vertex shader.\n", shaderName.c_str() );
					}
				}
				else
					eage::debug( "<ERROR> ShaderObject: [%s], No vertex shader has been found.\n", shaderName.c_str() );
			}
			// if this is fragment shader
			else if( strcmp( key, "FRAGMENT" ) == 0 )
			{
				...
				if( !shader.empty() )
				{
					...
					{
						eage::debug( "<ERROR> ShaderObject: [%s], failed to compile fragment shader.\n", shaderName.c_str() );
					}
				}
				else
					eage::debug( "<ERROR> ShaderObject: [%s], No fragment shader has been found.\n", shaderName.c_str() );
			}
		}
	}
	...
	if( _vertCom && _fragCom )
	{
		...
		{
			eage::debug( "<ERROR> ShaderObject: [%s], failed to link shader program.\n", shaderName.c_str() );
		}
	}
	else
	{
