...
sf::Vector2f eage::getResolutionScale()
{
	float x,y;
	x = RenderModule::getInstance()->_widthScale;
	y = RenderModule::getInstance()->_heightScale;
...
RenderModule::RenderModule()
{
	...
	_screenWidth	= RES_16_9_WIDTH;
	_screenHeight	= RES_16_9_HEIGHT;
	_widthScale		= 1.0f;
	_heightScale	= 1.0f;
...
void RenderModule::setDesignResolution( int width, int height )
{
	_designWidth = width;
...
void RenderModule::genScaleRatio()
{
	/*float	leftW	= _designWidth*1.0f,
			rightW	= _screenWidth*1.0f,
			leftH	= _designHeight*1.0f,
			rightH	= _screenHeight*1.0f;
	if( _designWidth > _screenWidth )
	{
		leftW = _screenWidth*1.0f;
		rightW = _designWidth*1.0f;
	}
	if( _designHeight > _screenHeight )
	{
		leftH = _screenHeight*1.0f;
		rightH = _designHeight*1.0f;
	}
	_widthScale = leftW/rightW;
	_heightScale = leftH/rightH;*/

	_widthScale = (float)_screenWidth/(float)_designWidth;
	_heightScale = (float)_screenHeight/(float)_designHeight;
}
