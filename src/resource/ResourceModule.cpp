...
bool ResourceModule::loadResFile( std::string filename, EAGE_RESOURCE_MODE mode )
{
	...
	while( !file.eof() )
	{
		...
		else if( strcmp( key, "[SAMPLE]" ) == 0 )
		{
			...
			while(1)
			{
				...
				{	
					if( mode == EAGE_FROM_MEMORY && data )
					{
						/*TextureTask* task = new TextureTask( data, size, atoi(name), GL_LINEAR );
						_texman->appendTexTask( task );*/
				...
				else if( ( strcmp( filter, "NEAREST" ) == 0 ) )
				{
					if( mode == EAGE_FROM_MEMORY && data )
					{
						/*TextureTask* task = new TextureTask( data, size, atoi(name), GL_NEAREST );
						_texman->appendTexTask( task );*/
		...
		else if( strcmp( key, "[SOUND]") == 0 )
		{
			...
			while(1)
			{
				...
				if( mode == EAGE_FROM_MEMORY )
				{
					...
					if( data )
					{
						/*SoundTask* task = new SoundTask( data, size, atoi(name) );
						_soundMan->appendSoundTask( task );*/
		...
		else if( strcmp( key, "[MUSIC]") == 0 )
		{
			...
			while(1)
			{
				...
				if( mode == EAGE_FROM_MEMORY )
				{
					...
					if( data )
					{
						/*MusicTask* task = new MusicTask( data, size, atoi(name) );
						_soundMan->appendMusicTask( task );*/
			...
			while(1)
			{
				...
				if( mode == EAGE_FROM_MEMORY )
				{
					...
					if( data )
					{
						...
						/*AnimationTask* task = new AnimationTask( data, atoi(name) );
						AnimationManager::getInstance()->appendAnimTask( task );*/
					}
				}	
			}
			continue;
	...
	// test lines
	//_soundMan->executeTasks();
	//AnimationManager::getInstance()->executeTasks();
	//_texman->executeTasks();
